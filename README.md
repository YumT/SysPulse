...
表示要素は次の 4 つ(この順でスロットに流し込まれる):

1. **メトリクス** — CPU / メモリ / GPU / イーサネット(負荷・速度・スパークライン)
2. **プロセス＆イベント** — プロセス表(残り全部) + イベント件数(下・2行ぶん)
3. **ドライブ** — ディスク(2 列。背景スパークライン付きセル)
4. **AI Usage** — Claude / Kimi の使用量ゲージ

右クリックメニューの「表示エリア」で領域数(レイアウト)を選び、
「表示要素」で出す要素を個別に切り替える。
領域数は 1×1=1 / 1×2=2 / 2×1=2 / 2×2=4 で、有効な要素が上の順に
スロットへ流し込まれる(領域数を超えた分は出ない)。
空いた下段スロットは真上の要素が縦に伸びて埋める。
デフォルトは 2×2 + 全要素表示で、その場合の配置は従来どおり
左上=メトリクス / 右上=プロセス＆イベント / 左下=ドライブ / 右下=AI Usage。
...
「常に手前に表示」「表示エリア」「表示要素」の状態を `window-state.json` に
保存し、次回起動時に復元する(画面外に保存されていた場合は位置だけ中央に戻す)。
...
- **表示エリア** — サブメニューからレイアウト(領域数)を 4 種から選択
  (**1×1 (領域 1)** / **1×2 (領域 2)** / **2×1 (領域 2)** / **2×2 (領域 4)**。
  ウィンドウサイズ自体は変えないが、最小サイズはモードに応じて緩む
  (1×1: 300x280、1×2: 560x280、2×1: 300x500、2×2: 560x500)ので
  小さいモードではその分小さく畳める(2×2 に戻すと最小サイズ未満なら自動で広がる)。
  状態は window-state.json に保存され、終了後も維持される)
- **表示要素** — サブメニューから出す要素を個別に切り替え
  (**CPU など (メトリクス)** / **プロセス＆イベント** / **ドライブ (ディスク)** /
  **AI Usage (Claude/Kimi)**。デフォルトはすべて表示。
  有効な要素がこの順で表示エリアのスロットに流し込まれ、
  領域数を超えた分は表示されない。空いた下段スロットは真上の要素が
  縦に伸びて埋める。状態は window-state.json に保存され、終了後も維持される)
- **常に手前に表示** — チェックでウィンドウを常に最前面に表示
  (状態は window-state.json に保存され、終了後も維持される)
- **Claude を表示 / Kimi を表示** — チェックで AI Usage 要素内の
  ゲージをプロバイダ個別に表示/非表示(デフォルトは両方表示。
  両方非表示のときは AI Usage 要素自体が出ず、そのぶん他の要素が
  詰めて表示される。状態は settings.json に保存され、終了後も維持される)
- **更新を確認** — 手動で最新リリースを確認(起動時の自動チェックと同じ流れ。
  更新があればダウンロードまで済ませてメニュー先頭に適用項目を挿入、
  最新版・確認失敗の場合はメッセージで通知)
...
- チェックは起動時のみ(定期ポーリングなし)。失敗時は何も出ない(黙殺)。
  起動時以外に確認したい場合は右クリックメニューの「更新を確認」を使う
  (同じ流れで DL・展開まで行い、最新版/失敗時はメッセージを表示)
