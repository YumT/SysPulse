...
/// <summary>
/// プロセス別 CPU / メモリ / ディスク I/O / GPU。
/// CPU は TotalProcessorTime 差分 ÷ 経過時間 ÷ 論理コア数(タスクマネージャーと同じ 0-100%)。
/// ディスク I/O は GetProcessIoCounters(psutil の io_counters と同じソース)。
/// GPU は PDH "\GPU Engine(*)\Utilization Percentage" をインスタンス名の pid_xxxx で
/// プロセス毎に合算(タスクマネージャーの GPU 列と同じソース。瞬時値)。
...
public sealed class ProcessMonitor : IDisposable
{
    ...
    private readonly PdhQuery _gpuPdh = new();
    private readonly IntPtr? _gpuUtilCounter;

    public ProcessMonitor()
    {
        // "pid_1800_luid_..._eng_0_engtype_3d" 形式のインスタンスを全展開。
        // GPU 非搭載/ドライバ非対応なら null → 全行「—」表示に回る。
        _gpuUtilCounter = _gpuPdh.AddCounter(@"\GPU Engine(*)\Utilization Percentage");
    }
    ...
    public List<ProcSample> Sample(int limit = 8)
    {
        ...
        foreach (var p in Process.GetProcesses())
        {
            ...
            rows.Add(new ProcSample
            {
                ...
                Disk = diskMbps,
                Gpu = gpuByPid is null ? null : (gpuByPid.TryGetValue(pid, out double g) ? g : 0.0),
            });
        }
    ...
    /// <summary>
    /// GPU Engine カウンタを PID 毎に合算。インスタンス名は
    /// "pid_1800_luid_0x..._phys_0_eng_0_engtype_3d" 形式。
    /// カウンタ自体が取れない環境では null(「—」表示に回す)。
    /// </summary>
    private Dictionary<int, double>? SampleGpuByPid()
    {
        if (_gpuUtilCounter is null)
            return null;
        _gpuPdh.Collect();
        var engines = _gpuPdh.GetWildcardValues(_gpuUtilCounter);
        ...
        foreach (var (instance, value) in engines)
        {
            ...
            if (end <= 4 || !int.TryParse(instance.AsSpan(4, end - 4), out int pid))
                continue;
            byPid[pid] = byPid.TryGetValue(pid, out double cur) ? cur + value : value;
        }
        // 複数エンジン合算で 100 を超えることがあるのでタスクマネージャー同様に丸める
        foreach (int pid in byPid.Keys.ToList())
            if (byPid[pid] > 100.0)
                byPid[pid] = 100.0;
        return byPid;
    }

    public void Dispose() => _gpuPdh.Dispose();
}
